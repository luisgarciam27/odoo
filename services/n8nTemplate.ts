...
export const DAILY_WORKFLOW_JSON = {
  ...
  "nodes": [
    ...
    {
      "parameters": {
        "jsCode": "const config = $node[\"Configuracion\"].json;\n// El nodo HTTP de n8n parsea el XML directamente en la raíz de $json\n// Quitamos el acceso a .body que causaba el error\nconst root = $json.methodResponse || $json;\nconst params = root.params || root.body?.params;\nconst responseValue = params?.param?.value;\nconst sessionsRaw = responseValue?.array?.data?.value || [];\n\n// Normalizamos sessionsRaw a un array si solo viene uno\nconst sessionsList = Array.isArray(sessionsRaw) ? sessionsRaw : [sessionsRaw];\n\nlet totalVenta = 0;\nlet msg = `📊 *REPORTE DE CAJA - LEMONBI*\\n🏢 *${config.empresaName}*\\n📅 ${config.fecha}\\n\\n`;\n\nif (sessionsList.length === 0 || !sessionsRaw) {\n  msg += \"⚠️ No se registraron cierres de caja para esta fecha.\";\n} else {\n  sessionsList.forEach(s => {\n    const members = s.struct?.member || [];\n    const membersArr = Array.isArray(members) ? members : [members];\n    \n    // Buscamos los valores de forma segura dentro del struct de Odoo\n    const getName = (arr) => arr.find(m => m.name === 'name')?.value?.string || 'Caja';\n    const getVal = (arr, key) => {\n       const m = arr.find(m => m.name === key)?.value;\n       return parseFloat(m?.double || m?.int || m?.i4 || 0);\n    };\n\n    const name = getName(membersArr);\n    const end = getVal(membersArr, 'cash_register_balance_end_real');\n    const start = getVal(membersArr, 'cash_register_balance_start');\n    const neta = end - start;\n    \n    totalVenta += neta;\n    msg += `📍 *${name}*\\n   • Venta: S/ ${neta.toFixed(2)}\\n   • Arqueo: S/ ${end.toFixed(2)}\\n\\n`;\n  });\n  msg += `💰 *TOTAL VENTA NETA: S/ ${totalVenta.toFixed(2)}*`;\n}\n\nreturn { json: { message: msg, phones: config.targetPhones } };"
      },
      "name": "Format Message",
      "type": "n8n-nodes-base.code",
      "typeVersion": 2,

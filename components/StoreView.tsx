...
const StoreView: React.FC<StoreViewProps> = ({ session, config, onBack }) => {
  ...
  // Limpiar URLs vacías del slider
  ...
  const handleFinishOrder = async () => {
    ...
    try {
      ...
      // Registrar pedido en Supabase para analítica de la plataforma
      ...
      // Redirigir a WhatsApp con resumen de compra
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-800 flex flex-col relative overflow-x-hidden selection:bg-brand-100">
      ...
      {/* HERO SLIDER RESTAURADO E INTERACTIVO */}
        ...
        <section className="w-full h-[280px] md:h-[500px] relative overflow-hidden bg-slate-200 z-10">
           {slideImages.map((img, idx) => (
             ...
             >
                ...
                {/* Contenido decorativo sobre el slider */}
      ...
      {/* PLANTILLA DE DETALLE DE PRODUCTO (MODAL MEJORADO) */}
      {selectedProduct && (
      ...
      {/* FOOTER GENERAL */}
      {!loading && (
        <footer className="text-white pt-32 pb-16 px-6 md:px-12 relative overflow-hidden" style={{ backgroundColor: secondaryColor }}>
           {/* Decoración Footer */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-20 mb-24">
                 <div className="space-y-10">
                    {config.footerLogoUrl ? (
                      <img src={config.footerLogoUrl} className="h-14 object-contain" alt="Footer Logo" />
                    ) : (
                      <div className="flex items-center gap-3">
                         <Citrus className="w-10 h-10 text-brand-500" />
                         <h2 className="text-3xl font-black uppercase tracking-tighter leading-none">{config.nombreComercial || config.code}</h2>
                      </div>
                    )}
                    <p className="text-slate-400 text-xs font-bold uppercase leading-relaxed max-w-xs">{config.footer_description || "Expertos en analítica y ventas integradas con Odoo para potenciar tu crecimiento empresarial."}</p>
                    <div className="flex gap-4">
                       {config.facebook_url && <a href={config.facebook_url} target="_blank" className="p-4 bg-white/5 rounded-2xl hover:bg-brand-500 hover:scale-110 transition-all border border-white/5"><Facebook className="w-5 h-5"/></a>}
                       {config.instagram_url && <a href={config.instagram_url} target="_blank" className="p-4 bg-white/5 rounded-2xl hover:bg-pink-500 hover:scale-110 transition-all border border-white/5"><Instagram className="w-5 h-5"/></a>}
                    </div>
                 </div>
                 <div className="space-y-10">
                    <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-brand-500 border-b border-white/10 pb-4">Explora</h3>
                    <ul className="space-y-6">
                       <li><button onClick={() => { setSelectedCategory('Todas'); window.scrollTo({top: 350, behavior: 'smooth'}); }} className="text-xs font-black text-slate-400 hover:text-white transition-all uppercase tracking-widest flex items-center gap-3"><ChevronRight className="w-3 h-3"/> Todo el Catálogo</button></li>
                       <li><button onClick={onBack} className="text-xs font-black text-slate-400 hover:text-white transition-all uppercase tracking-widest flex items-center gap-3"><ChevronRight className="w-3 h-3"/> Panel de Gestión</button></li>
                    </ul>
                 </div>
                 <div className="space-y-10">
                    <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-brand-500 border-b border-white/10 pb-4">Asistencia</h3>
                    <div className="flex items-center gap-5 group cursor-pointer">
                        <div className="p-5 bg-white/5 rounded-[2rem] group-hover:bg-brand-500 transition-all shadow-2xl border border-white/5"><MessageCircle className="w-7 h-7 text-white"/></div>
                        <div><p className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-1">WhatsApp</p><p className="text-lg font-black tracking-widest">{config.whatsappHelpNumber || 'Escríbenos'}</p></div>
                    </div>
                 </div>
                 <div className="space-y-10">
                    <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-brand-500 border-b border-white/10 pb-4">Certificación</h3>
                    <div className="bg-white/5 p-10 rounded-[3.5rem] border border-white/5 space-y-6 text-center shadow-inner">
                       <div className="flex items-center gap-5"><ShieldCheck className="w-8 h-8 text-brand-500"/><span className="text-[11px] font-black uppercase tracking-widest">Seguridad TLS</span></div>
                       <div className="flex items-center gap-5"><Clock className="w-8 h-8 text-brand-500"/><span className="text-[11px] font-black uppercase tracking-widest">Disponibilidad 24/7</span></div>
                    </div>
                 </div>
              </div>
              <div className="pt-16 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-10 opacity-30 text-[11px] font-black uppercase tracking-[0.3em]">
                 <div className="flex items-center gap-3">
                    <span>Power by</span>
                    <span className="text-white">Lemon BI</span>
                    <span>&</span>
                    <span className="text-white">Gaor System</span>
                 </div>
      ...
      {/* DRAWER DEL CARRITO Y CHECKOUT (COMPLETO) */}
      {isCartOpen && (
        <div className="fixed inset-0 z-[120] flex justify-end">
           ...
           <div className="relative bg-white w-full max-w-2xl h-full shadow-[0_0_100px_rgba(0,0,0,0.3)] flex flex-col p-8 md:p-12 overflow-y-auto animate-in slide-in-from-right duration-500 custom-scrollbar">
              ...
              {/* PASO 2: DATOS DEL CLIENTE Y ENTREGA */}
              {currentStep === 'details' && (

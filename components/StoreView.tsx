...
const StoreView: React.FC<StoreViewProps> = ({ session, config, onBack }) => {
  ...
  /**
   * Carga de productos con recuperación automática.
   */
  const fetchProducts = async () => {
    ...
    try {
      ...
      // Intentamos con dominio de activos y venta
      ...
      // RECUPERACIÓN AUTOMÁTICA: Si no hay productos, intentamos búsqueda global sin activos (para Odoo 14)
      if (!data || data.length === 0) {
        console.warn("[LemonBI] Recuperación Global de Catálogo activada...");
        ...
        {loading ? (
          ...
          <div className="py-24 text-center flex flex-col items-center gap-8 animate-in fade-in zoom-in">
             ...
             <div className="space-y-4 max-w-sm">
                ...
                <p className="text-[10px] font-bold text-slate-400 leading-relaxed uppercase tracking-widest px-8">Asegúrate de que los productos estén marcados como "Puede ser vendido" en Odoo.</p>
                <button onClick={fetchProducts} className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[9px] tracking-widest flex items-center gap-3 mx-auto shadow-xl hover:scale-105 transition-all"><RefreshCw className="w-4 h-4" /> Reintentar Conexión</button>
             </div>
          </div>
        ) : (

...
import { Palette, ImageIcon, Save, CheckCircle2, RotateCcw, MapPin, Plus, Trash2, HeartPulse, Sparkles, MessageSquareText, Stethoscope, Footprints, Pill, Briefcase } from 'lucide-react';
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Personaliza la experiencia según tu especialidad médica.</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 space-y-6">
            <h3 className="text-lg font-black text-slate-800 flex items-center gap-3">
              <Briefcase className="w-6 h-6 text-brand-500" /> Especialidad del Negocio
            </h3>
            <div className="grid grid-cols-3 gap-3">
               {[
                 {id: 'pharmacy', icon: Pill, label: 'Farmacia'},
                 {id: 'veterinary', icon: Stethoscope, label: 'Veterinaria'},
                 ...
                 <button 
                  ...
                  className={`flex flex-col items-center gap-3 p-5 rounded-[2rem] border-2 transition-all ${currentConfig.businessType === type.id ? 'border-brand-500 bg-brand-50' : 'border-slate-50 bg-white hover:border-slate-200'}`}
                 >
          ...
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 space-y-6">
            ...
            <div className="space-y-4">
              {(currentConfig.sedes_recojo || []).map(sede => (
                <div key={sede.id} className="p-5 bg-slate-50 rounded-2xl border border-slate-100 flex gap-4">
                   <div className="flex-1 space-y-3">
                     <input type="text" placeholder="Nombre Sede" className="w-full p-3 bg-white border border-slate-200 rounded-xl text-sm font-bold" value={sede.nombre} onChange={e => updateSede(sede.id, 'nombre', e.target.value)} />
              ...
              <button type="button" onClick={addSede} className="w-full p-5 border-2 border-dashed border-slate-200 rounded-3xl text-slate-400 font-bold text-xs hover:border-blue-300 hover:text-blue-500 transition-all">+ Añadir Sede</button>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 space-y-6">
            ...
            <div className="space-y-5">
              <input type="text" placeholder="Nombre Comercial" className="w-full p-4 bg-slate-50 rounded-2xl font-black uppercase" value={currentConfig.nombreComercial} onChange={e => setCurrentConfig({...currentConfig, nombreComercial: e.target.value})} />
              <div className="flex gap-4">
                ...
                <input type="text" placeholder="URL Logo" className="flex-1 p-4 bg-slate-50 rounded-2xl text-xs" value={currentConfig.logoUrl} onChange={e => setCurrentConfig({...currentConfig, logoUrl: e.target.value})} />
              </div>
              <textarea className="w-full p-4 bg-slate-50 rounded-2xl text-xs h-20" placeholder="Descripción corta de marca..." value={currentConfig.footer_description} onChange={e => setCurrentConfig({...currentConfig, footer_description: e.target.value})} />
            </div>
          </div>

          <div className="pt-4 space-y-4">
             {showSuccess && (
               <div className="bg-slate-900 text-white px-8 py-5 rounded-[2rem] flex items-center justify-center gap-4 animate-in slide-in-from-bottom-6">
                  <CheckCircle2 className="w-5 h-5 text-brand-400" />
                  <span className="text-xs font-black uppercase tracking-[0.2em]">¡Configuración Guardada!</span>
               </div>
             )}
             <button type="submit" disabled={isSaving} className="w-full py-7 text-white rounded-[2.5rem] font-black shadow-2xl hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-4 text-sm uppercase tracking-[0.2em]" style={{backgroundColor: brandColor}}>
                {isSaving ? <RotateCcw className="w-6 h-6 animate-spin" /> : <Save className="w-6 h-6" />}
                Actualizar Portal Premium
             </button>
          </div>
        </div>
      </form>
